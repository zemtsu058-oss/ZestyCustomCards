...
function s.negcon(e, tp, eg, ep, ev, re, r, rp)
    -- Kiểm tra chain đang phản hồi kích hoạt của quái Labrynth hoặc Normal Trap
    local re_card = re:GetHandler()
    ...
    -- Quái Labrynth hoặc Normal Trap của ta
    local is_labrynth = trig_card:IsSetCard(0xfb) -- setcode Labrynth
    local is_normal_trap = (trigger_re:GetType() == EFFECT_TYPE_ACTIVATE)
...
function s.spcon(e, tp, eg, ep, ev, re, r, rp)
    ...
    return trig_card:IsSetCard(0xfb) and trig_card:IsType(TYPE_MONSTER)
end
...
function s.defcon(e)
    ...
    local c = e:GetHandler()
    return c:IsPosition(POS_FACEUP_DEFENCE) or c:IsPosition(POS_FACEDOWN_DEFENCE)
end

function s.defval(e, re, r)
    -- Chặn phá hủy bởi hiệu ứng lá (REASON_EFFECT)
