...
6. **ALWAYS** trong operation của trigger/continuous effect dùng mẫu chuẩn `local c=e:GetHandler()` + check `c:IsRelateToEffect(e)`. **EDOPro KHÔNG tự load `script/constants.lua`** — nếu dùng định danh từ file đó (`SET_*`, helper...), bắt buộc có `Duel.LoadScript("constants.lua")` ở đầu script. **NEVER** gọi hàm API không thấy trong script official (hàm bịa → crash runtime; xem `script-test/phantom_apis.txt`). Chi tiết tại [`docs/agent-rules.md`](docs/agent-rules.md).
